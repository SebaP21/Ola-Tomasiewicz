...
const AnimatedGallery: React.FC<AnimatedGalleryProps> = ({ gallery }) => {
  ...
  // Używamy tablicy do przechowywania stanu kliknięcia dla każdego zdjęcia
  ...
  // Aktualizujemy stan kliknięcia, gdy galeria jest załadowana
  useEffect(() => {
    if (gallery) {
      setClickedStates(Array(gallery.length).fill(false)); // Inicjalizujemy stan kliknięcia
    }
  }, [gallery]);
  ...
  const handleClick = (index: number) => {
    setClickedStates((prev) => {
      ...
      newClickedStates[index] = !newClickedStates[index]; // Przełączamy stan dla klikniętego zdjęcia
    ...
    <div className='grid grid-cols-1 gap-1 lg:grid-cols-2'>
      {gallery?.map((picture, index) => (
        <div
          ...
          onTouchStart={() => handleClick(index)} // Obsługa kliknięcia na urządzeniach dotykowych
          onClick={() => handleClick(index)} // Obsługa kliknięcia
        >
          <Image
          ...
          {/* Animowany cień, tylko jeśli nie ma kliknięcia */}
          <div
            className={`absolute inset-0 shadow-layer ${
              shadowIndex === 0 && !clickedStates[index] ? "opacity-100" : "opacity-0"
            } shadow-right`}
          ></div>
          <div
            className={`absolute inset-0 shadow-layer ${
              shadowIndex === 1 && !clickedStates[index] ? "opacity-100" : "opacity-0"
            } shadow-left`}
          ></div>
          <div
            className={`absolute inset-0 shadow-layer ${
              shadowIndex === 2 && !clickedStates[index] ? "opacity-100" : "opacity-0"
            } no-shadow`}
          ></div>
        </div>
      ))}
